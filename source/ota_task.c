...
void ota_task(void *args)
{
    ...
    if (cy_ota_agent_start(&ota_network_params, &ota_agent_params, &ota_interfaces, &ota_context) != CY_RSLT_SUCCESS)
    {
...
cy_rslt_t connect_to_wifi_ap(void)
{
    ...
    for (conn_retries = 0; conn_retries < MAX_CONNECTION_RETRIES; conn_retries++)
    {
        ...
        cy_rtos_delay_milliseconds(WIFI_CONN_RETRY_DELAY_MS);
    }
...
cy_ota_callback_results_t ota_callback(cy_ota_cb_struct_t *cb_data)
{
    ...
    case CY_OTA_REASON_STATE_CHANGE:
        ...
        case CY_OTA_STATE_OTA_COMPLETE:
            ...
            if (cb_data->error == CY_RSLT_SUCCESS)
            {
                printf("OTA Completed without any error.\n");
                cb_result = cy_ota_storage_switch_to_new_image(1);
            }
            else
            {
                printf("OTA Completed with error = 0x%lX\n", (unsigned long)cb_data->error);
