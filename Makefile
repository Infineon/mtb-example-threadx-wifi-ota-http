...
APPNAME=mtb-example-threadx-wifi-ota-http
...
ifeq ($(OTA_SUPPORT),1)
  ...
  #  DEFINES+=ENABLE_MQTT_LOGS ENABLE_HTTP_CLIENT_LOGS ENABLE_AWS_PORT_LOGS ENABLE_SECURE_SOCKETS_LOGS
    ...
    # Required only for H1-CP based OTA test code
    ifeq ($(CY_BOOTLOADER),H1_CP)
        # Enable for JOB Flow
        DEFINES+=JOB_FLOW=1
        ...
        ifeq ($(OTA_MQTT_SUPPORT),1)
            # Enable required MQTT broker
            DEFINES+=MQTT_AWS_BROKER=1
            #DEFINES+=MQTT_ECLIPSE_BROKER=1
            #DEFINES+=MQTT_MOSQUITTO_BROKER=1
        endif
